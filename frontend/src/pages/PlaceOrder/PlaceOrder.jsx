...
const PlaceOrder = () => {
  ...
  const placeOrder = async (event) => {
    ...
    console.log('Placing order with data:', orderData); // Log order data
    ...
    try {
      ...
      if (response.data.success) {
        ...
        console.log('Redirecting to Stripe session:', session_url); // Log the Stripe session URL
        window.location.replace(session_url);
      } else {
        // Log the full error message from the API
        console.error('Error in order response:', response.data);
        alert(`Error placing order: ${response.data.message || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('Error making API request:', error); // Log the caught error
